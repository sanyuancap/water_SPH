...
float gWPoly6Scale = 0.0f;
float gWSpikyScale = 0.0f;
float gWViscosityScale = 0.0f;
float gWLucyScale = 0.0f;
...
// WPoly6
//==============================================================
inline float WPoly6(const tVector2 & r)
{
  float r2 = r.GetLengthSq();
...
inline tVector2 WPoly6Grad(const tVector2 & r)
{
  ...
  const float f = -6.0f * gWPoly6Scale;
  float a = gKernelH2 - r2;
  float b = f * a * a;
...
// WSpiky
//==============================================================
inline float WSpiky(const tVector2 & R)
{
  float r2 = R.GetLengthSq();
  if (r2 > gKernelH2) return 0.0f;
...
// WViscosity
//==============================================================
inline float WViscosity(const tVector2 & R)
{
  float r2 = R.GetLengthSq();
  if (r2 > gKernelH2) return 0.0f;
  static const float minR2 = 1.0E-12;
...
// WViscosityLap
//========================================================
inline float WViscosityLap(const tVector2 & R)
{
  float r2 = R.GetLengthSq();
  if (r2 > gKernelH2) return 0.0f;
  float r = sqrt(r2);
...
//==============================================================
inline float WLucy(const tVector2 & R)
{
  float r2 = R.GetLengthSq();
  if (r2 > gKernelH2) return 0.0f;
  float r = sqrt(r2);
  float a = (1.0f + 3.0f * r / gKernelH) * Cube(1 - r / gKernelH);
...
inline void AddBoundaryForce(const tVector2 & pos, tVector2 & force,
                             float dist, float scale)
{
  float delta = pos.y - gContainerY;
  if (delta < 0.0f) delta = 0.0f;
	if (delta < dist){
		float snum=delta/dist;
		force.y += scale * (1.0f - snum*snum);
	}
}
...
void ImposeBoundaryConditions()
{
  ...
  for (int i = gNParticles ; i-- != 0 ; )
  {
    ...
    if (gRectangle)
    {
      if (gRectangle->MovePointOutOfObject(gParticles[i].mR, _normal))
      {
        // do nothing extra
      }
    }
...
void SetObjectForces()
{
       ...
       iObjectParticle-- != 0 ; )
  {
    ...
    float sampleLen = edgeLen / nSamples;
    tSpatialGridIterator gridIter;
    for (int iSample = nSamples ; iSample-- != 0 ; )
    {
      // samples are at the mid positions of the edge element
      float fracStart = 1.0f - (0.5f + iSample)/nSamples;
      float fracEnd = 1.0f - fracStart;
  ...
  for (i = gNParticles ; i-- != 0 ; )
  {
         ...
         particle = gridIter.GetNext(*gSpatialGrid))
    {
      ...
      if (particle > &gParticles[i])
        continue;
      if (particle->mDensity > 0.000001f)
      {
        // pressure
        SubVector2(r, gParticles[i].mR, particle->mR);
        if (r.GetLengthSq() < gKernelH2)
...
{
  float minDist = 0.5f * gParticleRadius;
  float minDist2 = minDist*minDist;
  float resolveFrac = 0.1f;
  tVector2 delta;
  tSpatialGridIterator gridIter;
  for (int i = gNParticles ; i-- != 0 ; )
  {
         ...
         particle = gridIter.GetNext(*gSpatialGrid))
    {
      ...
      float deltaLenSq = delta.GetLengthSq();
      if (deltaLenSq > minDist2)
        continue;
      if (deltaLenSq > 0.000001)
      {
        float deltaLen = sqrt(deltaLenSq);
        float diff = resolveFrac * 0.5f * (deltaLen - minDist) / deltaLen;
        delta *= diff;
...
// DoFountain
//==============================================================
void DoFountain(float frac)
{
  static int fountainParticle = 0;
  tVector2 vel(0.0f, 15.0f);
...
void Initialise()
{
  ...
  if (gCreateObject)
  {
    float widthFrac = 0.2f;
    float heightFrac = 0.1f;
	...
	vts = (float*)calloc(gNRenderSamplesX*gNRenderSamplesY*4*2, sizeof(float));
	cls = (float*)calloc(gNRenderSamplesX*gNRenderSamplesY*4*4, sizeof(float));
}
...
void DrawContainer()
{
	...
	{
		gContainerX,gContainerY + gContainerHeight,
		gContainerX,gContainerY,
		gContainerX + gContainerWidth,gContainerY,
		gContainerX + gContainerWidth,gContainerY + gContainerHeight,
		gContainerX, gContainerY + gContainerHeight
	};
...
void DrawGridWater()
{
  ...
  float dx = gContainerWidth / (nx - 1);
  float dy = gContainerHeight / (ny - 1);

  static tArray2D<float> densities(nx, ny);
  ...
  float min = gRenderDensityMin;
  float max = gRenderDensityMax;
  ...
  for (i = nx-1 ; i-- != 0 ; )
  {
    for (j = ny-1 ; j-- != 0 ; )
    {
        float x = gContainerX + i * dx;
        float y = gContainerY + j * dy;
	...
	glLineWidth(5);
	glColorPointer(4, GL_FLOAT, 0, cls);
	glVertexPointer(2,GL_FLOAT,0, vts);
	glDrawArrays(GL_LINES, 0, nx*ny*4);
}
...
void DrawWater()
{
  if (gRenderParticles)
  {
	...
	float *vts = (float *)calloc((gNParticles+1)*2, sizeof(float));
    for (i = gNParticles ; i-- != 0 ; )
    {
		vts[n++]=gParticles[i].mR.x;vts[n++]=gParticles[i].mR.y;
    }
	glVertexPointer(2,GL_FLOAT,0, vts);
	glDrawArrays(GL_POINTS, 0, gNParticles+1);
...
void ReadConfig()
{
	...
	gRenderDensityMax = 900.0f;
	
	gDomainX = 2.0f;
	
	float gContainerWidthFrac = 0.8f;
	float gContainerHeightFrac = 0.5f;
	float gInitialFluidHeightFrac = 0.9f;
	...
	gContainerX = 0.5f * gDomainX - gContainerWidth;
	...
	gParticleRadius = sqrt(gContainerWidth * gInitialFluidHeight / 
						   (5.0f * gNParticles));
	float volume = gContainerWidth * gInitialFluidHeight;
